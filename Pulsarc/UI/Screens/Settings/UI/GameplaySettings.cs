...
namespace Pulsarc.UI.Screens.Settings.UI
{
    public class GameplaySettings : SettingsGroup
    {
        public GameplaySettings(Vector2 position) : base("Gameplay", position)
        {
            ...
            AddSetting("ApproachSpeed", new Slider("Approach Speed", "The speed at which arcs are approaching the crosshair", GetNextPosition(), "double", (int) Config.GetDouble(Name, "ApproachSpeed"), 1, 40));
            AddSetting("BackgroundDim", new Slider("Background Dim", "Opacity (in %) of the Dim applied to a beatmap's Background while playing", GetNextPosition(), "int", Config.GetInt(Name, "BackgroundDim"), 1));
